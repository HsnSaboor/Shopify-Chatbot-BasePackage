 ...
 * through secure postMessage communication with the chatbot iframe.
 * 
 ...
 * - Automatic cart update broadcasting
 * - Detailed logging
 */
...
(() => {
  ...
  // Configuration for the enhanced cart bridge
  ...
  // Logger for the enhanced cart bridge
  class EnhancedCartBridgeLogger {
    static log(level, context, ...args) {
      if (!CART_BRIDGE_CONFIG.debug) return;
      
      const timestamp = new Date().toISOString();
      const logMessage = `[${timestamp}] [ENHANCED-SHOPIFY-CART-BRIDGE] [${level.toUpperCase()}] [${context}]`;
  ...
  // Enhanced Cart API service that uses native Shopify domain
  class EnhancedShopifyCartAPI {
    static async addToCart(variantId, quantity = 1, properties = {}) {
      EnhancedCartBridgeLogger.info('CartAPI', 'Adding to cart:', { variantId, quantity, properties });
      
      // Validate inputs
      if (!EnhancedSecurityValidator.validateVariantId(variantId)) {
        throw new Error('Invalid variant ID');
      }
      
      if (!EnhancedSecurityValidator.validateQuantity(quantity)) {
        throw new Error('Invalid quantity');
      }
      
      const sanitizedProperties = EnhancedSecurityValidator.sanitizeInput(properties);
      ...
      try {
        ...
        EnhancedCartBridgeLogger.info('CartAPI', 'Successfully added to cart:', result);
        return {
          success: true,
          data: result,
        ...
        EnhancedCartBridgeLogger.error('CartAPI', 'Failed to add to cart:', error.message);
        throw error;
      }
    }

    static async getCart() {
      EnhancedCartBridgeLogger.debug('CartAPI', 'Fetching cart...');
      ...
      try {
        ...
        EnhancedCartBridgeLogger.debug('CartAPI', 'Cart retrieved:', formattedCart);
        return formattedCart;
      } catch (error) {
        EnhancedCartBridgeLogger.error('CartAPI', 'Failed to get cart:', error.message);
        throw error;
      }
    }

    static async updateCart(updates) {
      EnhancedCartBridgeLogger.info('CartAPI', 'Updating cart:', updates);
      
      try {
        const response = await this.makeRequest('POST', CART_BRIDGE_CONFIG.endpoints.updateCart, { updates });
        const result = await response.json();
        
        EnhancedCartBridgeLogger.info('CartAPI', 'Cart updated successfully');
        return {
          success: true,
          data: result,
          cart: await this.getCart()
        ...
        EnhancedCartBridgeLogger.error('CartAPI', 'Failed to update cart:', error.message);
        throw error;
      }
    }

    static async clearCart() {
      EnhancedCartBridgeLogger.info('CartAPI', 'Clearing cart...');
      
      try {
        const response = await this.makeRequest('POST', CART_BRIDGE_CONFIG.endpoints.clearCart);
        
        EnhancedCartBridgeLogger.info('CartAPI', 'Cart cleared successfully');
        return {
          success: true,
          cart: await this.getCart()
        };
      } catch (error) {
        EnhancedCartBridgeLogger.error('CartAPI', 'Failed to clear cart:', error.message);
        throw error;
      }
    }

    static navigateToCart() {
      EnhancedCartBridgeLogger.info('Navigation', 'Navigating to cart page');
      window.location.href = CART_BRIDGE_CONFIG.endpoints.cartPage;
    }

    static navigateToCheckout() {
      EnhancedCartBridgeLogger.info('Navigation', 'Navigating to checkout page');
  ...
  // Enhanced message handler for iframe communication
  class EnhancedCartBridgeMessageHandler {
    static handleMessage(event) {
      // Security validation
      if (!EnhancedSecurityValidator.validateMessage(event)) {
        EnhancedCartBridgeLogger.warn('Security', 'Message failed security validation');
        return;
      }

      // Validate origin
      if (!this.isAllowedOrigin(event.origin)) {
        EnhancedCartBridgeLogger.warn('Security', 'Message from unauthorized origin:', event.origin);
        return;
      }

      const { type, payload, messageId } = event.data;
      
      EnhancedCartBridgeLogger.debug('MessageHandler', 'Received message:', { type, payload, messageId });
      ...
      try {
        ...
        switch (type) {
          case 'CART_ADD_ITEM':
            ...
            const sanitizedVariantId = EnhancedSecurityValidator.sanitizeInput(payload.variantId);
            const sanitizedQuantity = payload.quantity || 1;
            const sanitizedProperties = EnhancedSecurityValidator.sanitizeInput(payload.properties || {});
            
            result = await EnhancedShopifyCartAPI.addToCart(
              sanitizedVariantId,
              sanitizedQuantity,
              sanitizedProperties
            );
          ...
          case 'CART_GET':
            result = await EnhancedShopifyCartAPI.getCart();
            break;

          case 'CART_UPDATE':
            const sanitizedUpdates = EnhancedSecurityValidator.sanitizeInput(payload.updates);
            result = await EnhancedShopifyCartAPI.updateCart(sanitizedUpdates);
            break;

          case 'CART_CLEAR':
            result = await EnhancedShopifyCartAPI.clearCart();
            break;

          case 'NAVIGATE_TO_CART':
            EnhancedShopifyCartAPI.navigateToCart();
            result = { success: true, action: 'navigation_started' };
            break;

          case 'NAVIGATE_TO_CHECKOUT':
            EnhancedShopifyCartAPI.navigateToCheckout();
            result = { success: true, action: 'navigation_started' };
            break;
        ...
        EnhancedCartBridgeLogger.error('MessageHandler', 'Error processing message:', error.message);
    ...
    static sendResponse(source, origin, messageId, data) {
      ...
      if (!source || !source.postMessage) {
        EnhancedCartBridgeLogger.warn('MessageHandler', 'Invalid message source, cannot send response');
        return;
      }
      ...
      EnhancedCartBridgeLogger.debug('MessageHandler', 'Sending response:', response);
  ...
  // Enhanced initialize the cart bridge
  class EnhancedShopifyCartBridge {
    static init() {
      EnhancedCartBridgeLogger.info('Bridge', 'Initializing Enhanced Shopify Cart Bridge v' + CART_BRIDGE_CONFIG.version);
      
      // Add message event listener
      window.addEventListener('message', (event) => {
        EnhancedCartBridgeMessageHandler.handleMessage(event);
      });
      ...
      window.ENHANCED_SHOPIFY_CART_BRIDGE_READY = true;
      
      EnhancedCartBridgeLogger.info('Bridge', 'Enhanced cart bridge initialized successfully');
    ...
    static initCartCountTracking() {
      ...
      window.fetch = async function(...args) {
        ...
        if (typeof url === 'string' && (url.includes('/cart/') || url.includes('cart.js'))) {
          ...
          setTimeout(() => {
            EnhancedShopifyCartBridge.broadcastCartUpdate();
          }, 100);
        }
    ...
    static async broadcastCartUpdate() {
      try {
        const cart = await EnhancedShopifyCartAPI.getCart();
        const message = {
          type: 'CART_UPDATED',
          data: cart,
          timestamp: new Date().toISOString()
        ...
        EnhancedCartBridgeLogger.warn('Bridge', 'Failed to broadcast cart update:', error.message);
      }
    }
    ...
    static notifyBridgeReady() {
      const message = {
        ...
        data: {
          version: CART_BRIDGE_CONFIG.version,
          storeInfo: EnhancedCartBridgeMessageHandler.getStoreInfo()
        },
        timestamp: new Date().toISOString()
      };
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
      EnhancedShopifyCartBridge.init();
    });
  } else {
    EnhancedShopifyCartBridge.init();
  }
  ...
  window.EnhancedShopifyCartBridge = EnhancedShopifyCartBridge;
  
  EnhancedCartBridgeLogger.info('Bridge', 'Enhanced Shopify Cart Bridge script loaded successfully');
